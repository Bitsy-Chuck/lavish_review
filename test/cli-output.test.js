...
test("design output prints copy-pasteable local /design URLs so agents can opt in to DaisyUI", () => {
  ...
  assert.match(output.diagram_tooling.mermaid_cdn_snippet, /startOnLoad: false/);
  assert.match(output.diagram_tooling.mermaid_cdn_snippet, /securityLevel: "strict"/);
  // De-CDN work vendors Mermaid locally, so the snippet imports the packaged module, not a CDN URL.
  assert.equal(output.diagram_tooling.cdn_urls.mermaid, "/design/mermaid.esm.min.mjs");

  // The snippet re-renders on page-theme flips (theme-sync, #162) and, when the artifact themes with
  // DaisyUI, derives Mermaid's palette from the DaisyUI tokens. Mermaid runs color math (khroma) over
  // themeVariables and khroma throws "Unsupported color format" on both oklch() - what DaisyUI 5 emits -
  // and on var(). Handing either straight to Mermaid breaks every diagram, so the snippet must resolve
  // tokens to plain sRGB hex in the browser first. Guard the shape rather than the exact colors, ignoring
  // the snippet's own explanatory comments (which necessarily name the formats they warn against).
  ...
  // Upstream #162 theme-sync machinery: re-renders diagrams when the page theme changes.
  assert.match(output.diagram_tooling.mermaid_cdn_snippet, /mermaid\.run/);
  assert.match(output.diagram_tooling.mermaid_cdn_snippet, /prefers-color-scheme: dark/);
...
test("design output emits a theme-aware Mermaid init that re-renders on page-theme change", () => {
  ...
  // The old bug: a single hardcoded Mermaid theme that ignores the page theme. The
  // theme is now chosen dynamically each render - DaisyUI artifacts get "base" plus
  // token-derived themeVariables, everything else gets Mermaid's built-in dark/default -
  // so it is never a fixed constant that ignores the page.
  assert.match(snippet, /hasDaisyTokens \? "base"/);
...
test("theme-aware Mermaid snippet serializes rapid theme-change renders", async () => {
  ...
  new Function("mermaid", "window", "document", "MutationObserver", "getComputedStyle", "console", snippet)(
    ...
    (element) => ({
      ...
      // This fixture is a non-DaisyUI page: no design tokens resolve, so the snippet
      // takes the built-in dark/default path this test exercises.
      getPropertyValue: () => "",
    }),
