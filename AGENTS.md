...
- The in-iframe layout audit (`src/artifact-sdk.js`) has three easy-to-reintroduce failure modes, all covered by dedicated tests:
  ...
  - A finding carries two selectors and they are not interchangeable. `selector` is for a human to read, so `selector()` caps it at 5 ancestors and short-circuits on an id; `identity` is `identitySelector()`'s full tag-plus-`:nth-of-type` path to the root, and it is the only thing safe to key on. Keying on the display selector collapses two distinct elements that share a capped path into one finding - the second is dropped in the SDK's own dedupe and never reaches the agent - and inside a Mermaid diagram it also changes on every re-render, because the id short-circuit picks up an id Mermaid regenerates. `layoutWarningKey` (`session-store.js`, also used by the durable log) is the single spelling of that key and falls back to `selector` only for findings recorded before identity existed. `identity` is omitted when it would equal `selector`.
