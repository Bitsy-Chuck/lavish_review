...
export const PLAYBOOKS = [
  {
    ...
    structure: [
      ...
      "Add accTitle and accDescr to every Mermaid diagram. Its visible caption must state the conclusion in prose, not merely name the figure.",
    ],
    design_rules: [
      ...
      "At 390px, allow no more than 4 nodes across the widest rank. Otherwise wrap the diagram in overflow-x:auto and set Mermaid useMaxWidth:false; useMaxWidth:true silently shrinks wide diagrams until their text is unreadable, so ordinary overflow auditing cannot protect them.",
      ...
      'Mermaid configuration can be global in mermaid.initialize(...) or local as the first line of one diagram: %%{init: {"look":"handDrawn","layout":"elk","flowchart":{"useMaxWidth":false}}}%%. `look: handDrawn` requests sketch geometry (support varies by type); `layout` selects the layout engine for supporting diagrams; the %%{init}%% directive scopes overrides to that diagram. Never put theme or themeVariables there: the design snippet owns theme-derived colors.',
      "For a sketch treatment, use look: handDrawn and a fixed handDrawnSeed. It applies to flowchart, state-v2, class-v2, ER, requirement, and kanban, but not sequence, block, or architecture.",
    ],
    pitfalls: [
    ...
    syntax_examples: [
      ...
      "kanban — `kanban\\n  accTitle: Delivery board\\n  accDescr: Work is grouped into queued, active, and done columns.\\n  queued[Queued]\\n    spec[Write spec]\\n  active[Active]\\n    build[Build UI]\\n  done[Done]\\n    audit[Run audit]`",
      "radar — `radar-beta\\n  title Quality profile\\n  axis speed, clarity, access, safety, reach\\n  curve current{3, 4, 5, 4, 2}\\n  curve target{4, 5, 5, 5, 4}` (add an accessible caption and adjacent values; radar syntax has no accTitle/accDescr directives)",
      'treemap — `treemap-beta\\n  "Product"\\n    "Core": 55\\n    "Growth": 25\\n    "Platform": 20` (add an accessible caption and adjacent hierarchy; treemap syntax has no accTitle/accDescr directives)',
      ...
      "architecture — `architecture-beta\\n  group cloud(cloud)[Cloud]\\n  service api(server)[API] in cloud\\n  service db(database)[Data] in cloud\\n  api:R --> L:db` (add an accessible caption and adjacent plain text; architecture syntax has no accTitle/accDescr directives)",
      'C4 — `C4Context\\n  title System context\\n  Person(user, "User", "Reviews work")\\n  System(app, "Lavish", "Shows artifacts")\\n  System_Ext(repo, "Repository", "Stores source")\\n  Rel(user, app, "Reviews")\\n  Rel(app, repo, "Reads")` (add an accessible caption and adjacent plain text; C4 syntax has no accTitle/accDescr directives)',
