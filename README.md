...
- **Export and sharing** - `lavish-axi export` writes `<name>.export.html` by inlining local assets only, stripping the annotation SDK, and leaving remote CDN/font references as links that still need network access.
  `lavish-axi share` publishes the same HTML to [ht-ml.app](https://ht-ml.app), a third-party hosting service not part of Lavish, but inlines local assets only while the page stays under ht-ml.app's 6.3 MB request cap and uploads the rest as separate site files of up to 4.5 MB each.
  A local file that is bigger than 4.5 MB, or that the page references only through `./`, `poster`, `<track>`, or CSS `url()`, cannot be hosted there; share still publishes the page and reports the file as an unresolved local asset with the reason, in the command output and in the browser dialog.
  `uploaded_assets` lists the files that were uploaded separately.
