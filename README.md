...
- **Private offline export** - The Export standalone HTML button and `lavish-axi export` always create a sanitized copy named `artifact.export.html`.
  Local media and downloadable JSON, text, and shell files are embedded. Download links work offline.
  Recognized credentials, account and request IDs, email addresses, local paths, service names, and external URLs are redacted.
  PNG, JPEG, WebP, and MP4 provenance metadata is stripped. Unsupported attachments and unresolved references are removed with notices.
  The source artifact remains unchanged. No remote resource is fetched during export.
  Private export caps default to 64 MB per asset and 256 MB per bundle. Override them with `LAVISH_AXI_EXPORT_MAX_ASSET_BYTES` and `LAVISH_AXI_EXPORT_MAX_BUNDLE_BYTES`.
  Mark private elements with `data-export-private`. Add private names with `<meta name="lavish-export-redact" content='["Private name"]'>`.
  Use `data-export-src`, `data-export-href`, or `data-export-poster` to select a redacted local media copy during export.
  These alternatives use the same directory and symlink confinement as normal assets.
  Automatic rules cannot identify every private name, encoded secret, or detail visible in media. Review the exported copy before sharing.
  Removed remote libraries or scripts can reduce interactivity. Use local dependencies for an offline artifact.
- **Hosted sharing** - `lavish-axi share` publishes the source artifact to [ht-ml.app](https://ht-ml.app), a third-party service.
  To publish a sanitized copy, export first and share the exported HTML.
  Sharing inlines assets within the 6.3 MB page cap and uploads other files up to 4.5 MB each.
  Larger files and unsupported references are reported as unresolved assets. `uploaded_assets` lists separately uploaded files.
  Publishing is public by default, or password-protected with `--password`. The response includes a secret `update_key` for later management.
  Use `--token` or `LAVISH_AXI_HTML_APP_TOKEN` for an optional bearer token. `LAVISH_AXI_HTML_APP_API_URL` overrides the API base.
  Generic share bundling retains remote references and uses 10 MB per-asset and 25 MB per-bundle limits before the hosting cap applies.
  Lavish sets no response CSP. Local reads remain confined and size-capped.
...
- **Public origin behind a proxy** - Set `LAVISH_AXI_PUBLIC_ORIGIN` to an absolute origin (such as `https://lavish.example`) to build session links from it instead of `http://<link-host>:<port>`. Use this when a reverse proxy terminates TLS on a different scheme, host, or port than the one the server binds, where the default link would name a port that is not reachable through the proxy. Only `http`/`https` values are accepted and any path is dropped; an unset or unusable value falls back to the host-and-port link rather than emitting a broken one.
...
| `lavish-axi export <html-file>` | Write a sanitized offline HTML copy with embedded local media and downloadable files. External references and recognized private information are removed.                                                                                                                                                                                                                                        |
| `lavish-axi share <html-file>`  | Publish the artifact to [ht-ml.app](https://ht-ml.app), a third-party host not part of Lavish, and print a visitable URL plus a secret update key; local assets are inlined while the page fits ht-ml.app's 6.3 MB cap and uploaded as separate site files (up to 4.5 MB each) when they do not; shares are public by default, and `--password` makes viewers enter the password before viewing. |
